(function () {
  ...
  window.startOfflineFallback = function startOfflineFallback() {
    ...
    function buildCrate(resetView) {
      ...
      if (totalBeamsEl) totalBeamsEl.textContent = String(12 + totalSupports);
      if (totalLengthEl) totalLengthEl.textContent = (mainLengthMeters + supportLengthMeters).toFixed(2);
      if (faceSummaryEl) faceSummaryEl.innerHTML = summaries.join('<br>');
      if (modeBadgeEl) modeBadgeEl.textContent = activeFaces + ' sisi aktif';
    ...
    function draw() {
      ...
      faces.forEach(function (face) {
        ...
        var base = face.box.type === 'support' ? '#ddb67e' : '#c98f4d';
        ...
        if (face.faceIndex === 3 || face.faceIndex === 1) {
          ctx.save();
          ctx.clip();
