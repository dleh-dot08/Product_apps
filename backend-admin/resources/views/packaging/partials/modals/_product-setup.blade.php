<!-- Modal Product Setup (Wizard) -->
<div class="modal fade" id="productSetupModal" tabindex="-1" aria-labelledby="productSetupModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content border-0 shadow-lg">
            <div class="modal-header bg-light border-bottom pb-3">
                <h5 class="modal-title text-dark fw-bold" id="productSetupModalLabel">Add / Edit Data (Product Setup)</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>

            <div class="modal-body p-3" style="max-height: 70vh; overflow-y: auto;">
                <div class="tab-content" id="setupWizardContent">
                    <!-- Step 1: Sales Order Info -->
                    <div class="tab-pane fade show active" id="step1" role="tabpanel" aria-labelledby="step1-tab">
                        @php
                            ...
                            if ($hasJob) {
                                $initialSO = $calculation->job->no_so ?? '';
                                $initialCustomer = $calculation->job->customer ?? '-';
                                $initialDelivery = $calculation->job->date_delivery ?? '-';
                                $initialAddress = $calculation->job->address ?? '-';
                                
                                $initialProduct = $calculation->no_product ?? '';
                                $initialDesc = $calculation->desc_product ?? '-';
                                
                                // Coba ambil qty dari JSON daftar_iso_item_json di tabel packaging_jobs
                                if (!empty($calculation->job->daftar_iso_item_json)) {
                                    $items = is_string($calculation->job->daftar_iso_item_json) 
                                        ? json_decode($calculation->job->daftar_iso_item_json, true) 
                                        : $calculation->job->daftar_iso_item_json;
                                        
                                    if (is_array($items)) {
                                        foreach ($items as $item) {
                                            $itemNo = $item['no_barang'] ?? $item['item_no'] ?? $item['part_no'] ?? '';
                                            if ($itemNo == $initialProduct) {
                                                $initialQtyOrder = $item['qty'] ?? $item['qty_order'] ?? '-';
                                                $initialQtyRemaining = $item['sisa_kirim'] ?? $item['qty_remaining'] ?? '-';
                                                break;
                                            }
                                        }
                                    }
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // --- END AUTO-FILL LOGIC ---
                    ...
                    <!-- Step 2: Placeholder for now -->
                    <div class="tab-pane fade" id="step2" role="tabpanel" aria-labelledby="step2-tab">
                        <div class="text-center py-5">
                            <div class="spinner-border text-primary mb-3" role="status" style="display:none;" id="step2_loading">
                                <span class="visually-hidden">Loading...</span>
                            </div>
                            <h6 class="text-muted">Step 2 will be configured later...</h6>
                            <p class="small text-secondary mb-0">Proceed from Step 1 first.</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="modal-footer bg-light border-top-0 d-flex justify-content-between">
                <button type="button" class="btn btn-outline-secondary px-4 d-none" id="btn_prev_step">Back</button>
                <div class="ms-auto">
                    <button type="button" class="btn btn-secondary px-4" data-bs-dismiss="modal">Cancel</button>
                    <button type="button" class="btn btn-primary px-4" id="btn_next_step">Next Step</button>
                </div>
            </div>
        </div>
    </div>
            ...
            btnNext.addEventListener('click', function() {
                ...
                if (document.getElementById('step1').classList.contains('active')) {
                    ...
                    if (!dropdown || !dropdown.value) {
                        alert('Please select a product first before proceeding.');
                        return;
                    }
                ...
                // Save Final Step
                    ...
                    alert('Data saved! (To be implemented)');
                }
            });
