(function () {
  ...
  const totalCoverPiecesEl = document.getElementById('totalCoverPieces');
  const coverAreaEl = document.getElementById('coverArea');
  ...
  coverMaterial.color = new THREE.Color(0xd59a59);
  coverMaterial.roughness = 0.76;
  const plywoodMaterial = frameMaterial.clone();
  plywoodMaterial.color = new THREE.Color(0xe4c394);
  ...
  function clearGroup(group) {
    while (group.children.length) {
      ...
      if (child.material && child.material.dispose && child.material !== frameMaterial && child.material !== supportMaterial && child.material !== coverMaterial && child.material !== plywoodMaterial) {
  ...
  function readCover(face) {
    return {
      enabled: document.getElementById(face.id + 'CoverEnabled').checked,
      type: document.getElementById(face.id + 'CoverType').value,
      orientation: document.getElementById(face.id + 'CoverOrientation').value
    };
  }
  ...
  function makeBoardLayout(crossSpan, boardWidth, gap, isFull) {
    ...
    if (isFull) {
      ...
      for (let i = 0; i < count; i += 1) positions.push(-crossSpan / 2 + pieceCross / 2 + i * pieceCross);
    } else {
      count = Math.max(1, Math.min(80, Math.floor((crossSpan + gap) / (safeWidth + gap))));
      pieceCross = Math.min(safeWidth, crossSpan / count);
      const remaining = Math.max(0, crossSpan - count * pieceCross);
      const actualGap = remaining / (count + 1);
      for (let i = 0; i < count; i += 1) positions.push(-crossSpan / 2 + actualGap + pieceCross / 2 + i * (pieceCross + actualGap));
    }
  ...
  function addCoverForFace(face, state, L, W, H, inputs) {
    ...
    const boardWidth = Math.max(0.04, inputs.boardWidth / 1000);
    const boardThickness = Math.max(0.005, inputs.boardThickness / 1000);
    const halfGap = Math.max(0.005, inputs.halfGap / 1000);
    const plywoodThickness = Math.max(0.003, inputs.plywoodThickness / 1000);
    const isPlywood = state.type === 'plywood';
    const isFull = state.type === 'full';
    const material = isPlywood ? plywoodMaterial : coverMaterial;
    const label = state.type === 'half' ? 'Papan Setengah' : (state.type === 'full' ? 'Papan Full' : 'Triplex');

    if (isPlywood) {
      let area;
      if (face.id === 'front' || face.id === 'back') {
        const z = face.id === 'front' ? W / 2 + plywoodThickness / 2 : -W / 2 - plywoodThickness / 2;
        addBeam(new THREE.Vector3(L, H, plywoodThickness), new THREE.Vector3(0, H / 2, z), material, face.name + ' triplex');
        area = L * H;
      } else if (face.id === 'right' || face.id === 'left') {
        const x = face.id === 'right' ? L / 2 + plywoodThickness / 2 : -L / 2 - plywoodThickness / 2;
        addBeam(new THREE.Vector3(plywoodThickness, H, W), new THREE.Vector3(x, H / 2, 0), material, face.name + ' triplex');
        area = W * H;
      } else {
        const y = face.id === 'top' ? H + plywoodThickness / 2 : -plywoodThickness / 2;
        addBeam(new THREE.Vector3(L, plywoodThickness, W), new THREE.Vector3(0, y, 0), material, face.name + ' triplex');
        area = L * W;
      }
      return { pieces: 1, area: area, length: 0, summary: face.name + ': ' + label };
    }
    ...
    const layout = makeBoardLayout(crossSpan, boardWidth, halfGap, isFull);
    layout.positions.forEach(function (pos) {
      if (face.id === 'front' || face.id === 'back') {
        const z = face.id === 'front' ? W / 2 + boardThickness / 2 : -W / 2 - boardThickness / 2;
        if (state.orientation === 'H') addBeam(new THREE.Vector3(L, layout.pieceCross, boardThickness), new THREE.Vector3(0, H / 2 + pos, z), material, face.name + ' papan horizontal');
        else addBeam(new THREE.Vector3(layout.pieceCross, H, boardThickness), new THREE.Vector3(pos, H / 2, z), material, face.name + ' papan vertikal');
      } else if (face.id === 'right' || face.id === 'left') {
        const x = face.id === 'right' ? L / 2 + boardThickness / 2 : -L / 2 - boardThickness / 2;
        if (state.orientation === 'H') addBeam(new THREE.Vector3(boardThickness, layout.pieceCross, W), new THREE.Vector3(x, H / 2 + pos, 0), material, face.name + ' papan horizontal');
        else addBeam(new THREE.Vector3(boardThickness, H, layout.pieceCross), new THREE.Vector3(x, H / 2, pos), material, face.name + ' papan vertikal');
      } else {
        const y = face.id === 'top' ? H + boardThickness / 2 : -boardThickness / 2;
        if (state.orientation === 'H') addBeam(new THREE.Vector3(L, boardThickness, layout.pieceCross), new THREE.Vector3(0, y, pos), material, face.name + ' papan arah panjang');
        else addBeam(new THREE.Vector3(layout.pieceCross, boardThickness, W), new THREE.Vector3(pos, y, 0), material, face.name + ' papan arah lebar');
      }
    });

    return {
      pieces: layout.count,
      area: layout.count * layout.pieceCross * longSpan,
      length: layout.count * longSpan,
      summary: face.name + ': ' + label + ' ' + state.orientation + ' (' + layout.count + ')'
    };
  }
  ...
  function buildCrate(resetCamera) {
    ...
    FACE_CONFIG.forEach(function (face) {
      const coverState = readCover(face);
      if (coverState.enabled) activeCovers += 1;
      const result = addCoverForFace(face, coverState, L, W, H, inputs);
      totalCoverPieces += result.pieces;
      totalCoverArea += result.area;
      coverLengthMeters += result.length;
      coverSummaries.push(result.summary);
    });
    ...
    totalCoverPiecesEl.textContent = String(totalCoverPieces);
    coverAreaEl.textContent = totalCoverArea.toFixed(2);
    totalBeamsEl.textContent = String(12 + totalSupports + totalCoverPieces);
    totalLengthEl.textContent = (mainLengthMeters + supportLengthMeters + coverLengthMeters).toFixed(2);
    faceSummaryEl.innerHTML = '<strong>Penyangga</strong><br>' + summaries.join('<br>') + '<br><br><strong>Penutup</strong><br>' + coverSummaries.join('<br>');
    modeBadgeEl.textContent = activeFaces + ' penyangga • ' + activeCovers + ' penutup';
  ...
  function resetInputs() {
    ...
    FACE_CONFIG.forEach(function (face) {
      ...
      document.getElementById(face.id + 'CoverEnabled').checked = true;
      document.getElementById(face.id + 'CoverType').value = 'half';
      document.getElementById(face.id + 'CoverOrientation').value = 'H';
    });
    buildCrate(true);
  }
